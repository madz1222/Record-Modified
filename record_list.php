...
<?php $val = "record_list"?>
<form method="POST" action="" id="multi-trash-form">
    <div class="col-sm-12">
        <div class="card card-outline">
            <div class="card-header">
                <div class="card-tools">
                        <a class="btn btn-block btn-sm btn-secondary btn-flat" href="./index.php"><i class="fas fa-arrow-left"></i> Back </a>
                </div>
                <div class="card-tools mr-3">
                    <a class="btn btn-block btn-sm btn-warning btn-flat" href="./index.php?page=new_student_record"><i class="fa fa-plus"></i> Add New</a>
                </div>
                <div class="card-tools mr-3">
                    <a class="btn btn-block btn-sm btn-info btn-flat" href="./index.php?page=import_excel"><i class="fa fa-plus"></i> Import Excel</a>
                </div>	
                <div class="card-tools mr-3">
                    <a class="btn btn-block btn-sm btn-info btn-flat" href="ajax.php?action=export_excel"><i class="fa fa-download"></i> Export Excel</a>
                </div>
                ...
                <div>
                <b style='color:white' >Record List</b>
                </div>
...
<!--
				...
				<thead>
				        ...
				        <th>Action &nbsp;</th>
						<?php if($_SESSION['login_type'] == 1 ): ?>		
							<th class="clerk-id">Position ID &nbsp;</th>
				...
				<tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="./index.php?page=update_record&id=<?php echo $row['id'] ?>&val=<?php echo $val ?>" class="btn btn-primary btn-flat">
...
<script>
    $(document).ready(function() {
        ...
        $(".multi-trash-btn").click(function(){
            ...
            if(selected_record_ids.length > 0){
                if(confirm("Are you sure you want to delete the selected records?")){
                    trash_multiple_records (selected_record_ids);
                    start_load()
                }
            }else{
        ...
        $(document).on('click', '.export_excel', function() {
            _conf("Are you sure to export records?", "export_excel");
        });
    ...
    function export_excel() {
        $.ajax({
            url: 'ajax.php?action=export_excel',
            method: 'POST',
            success: function(resp) {
                if (resp == 1) {
                    alert_toast("Record Successfully Exported", 'success');
                    setTimeout(function() {
                        location.reload();
                    }, 500);
                } else if (resp == 2) {
                    alert_toast("Error Exporting Records", 'danger');
                    setTimeout(function() {
                        location.reload();
                    }, 1000);
                }
